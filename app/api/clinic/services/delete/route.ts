...
import { applyDeleteServiceToPersistedState } from "@/lib/apply-service-commands";
import {
...
/** Command API: удалить услугу без полного PUT snapshot. */
export async function POST(request: Request) {
  ...
  if (!serviceId) {
    return NextResponse.json(
      { ok: false, error: "Не указана услуга" },
      { status: 400, headers: APPOINTMENT_CMD_HEADERS }
    );
  }

  return saveAppointmentCommandResult(auth.clinicId, (state) => {
    const applied = applyDeleteServiceToPersistedState(state, serviceId);
