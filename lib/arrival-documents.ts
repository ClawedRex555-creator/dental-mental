...
Мне разъяснены цели, методы, возможные риски и последствия оказания медицинских услуг.${t["doctor.name"] !== "—" ? ` Лечащий врач: ${t["doctor.name"]}${t["doctor.specialization"] !== "—" ? ` (${t["doctor.specialization"]})` : ""}.` : ""}`;
  ...
  if (doc.kind === "health_card") {
    return `Карточка здоровья пациента

Пациент: ${who}
Адрес: ${t["patient.address"]}
Телефон: ${t["patient.phone"]}
СНИЛС: ${t["patient.snils"]}
${t["doctor.name"] !== "—" ? `Врач приёма: ${t["doctor.name"]}` : ""}

Аллергии / непереносимости: _______________________________________________

Хронические заболевания: _________________________________________________

Принимаемые препараты: ___________________________________________________
