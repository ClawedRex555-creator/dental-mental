...
export function DoctorModal({ open, onOpenChange, member }: DoctorModalProps) {
  const { addDoctor, updateDoctor, cabinets, doctors } = useClinicStore();
  ...
  const handleSave = () => {
    ...
    if (!isEdit) {
      ...
      if (authPassword.length < 6) {
        toast.error("Пароль для входа — не менее 6 символов");
        return;
      }
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          toast.error(data.error ?? "Сотрудник создан, но учётная запись не создана");
          return;
        }
        toast.success("Сотрудник и учётная запись для входа созданы");
      } catch {
        toast.error("Сотрудник создан, но не удалось создать учётную запись");
      }
    })();
