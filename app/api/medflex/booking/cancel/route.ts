...
import { cancelMedflexBooking } from "@/lib/medflex/booking.server";
...
export async function POST(request: Request) {
  ...
  try {
    body = (await request.json()) as { claim_id?: string };
  } catch {
  ...
  const result = await cancelMedflexBooking(auth.clinicId, body.claim_id.trim());
