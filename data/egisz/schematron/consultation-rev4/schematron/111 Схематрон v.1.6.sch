<!-- Схематрон для СЭМД "Протокол консультации в рамках диспансерного наблюдения (CDA) Редакция 4" -->
...
-->
    ...
    <pattern>
        ...
        <rule context="ClinicalDocument[code[@code='85']]">
            <assert test="count(participant[@typeCode='REF'])>=0 and count(participant[@typeCode='REF'])&lt;=1">У1-28. Элемент ClinicalDocument должен иметь 1 элемент participant.</assert>
        </rule>
    ...
    <pattern>
        <rule context="ClinicalDocument">
            <assert test="count(inFulfillmentOf)>=0 and count(inFulfillmentOf)&lt;=1">У1-29. Элемент ClinicalDocument должен иметь 1 элемент inFulfillmentOf.</assert>
        </rule>
