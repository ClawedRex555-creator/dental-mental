...
- `POST /v2/services/send_schedule/` (официальная схема):
  - обязательны блоки `services` (`basic` + `additional`), `doctors`, `devices`;
  - у каждой услуги в `basic`: `name`, `category`, `price`, `duration`,
    `additional_services` (массив `{id, required}` или `[]`), `intervals` (можно `[]`);
  - `government_code` — опционально (у нас из кода НМУ или `null`);
  - `additional` на уровне `services` — словарь доп. услуг (у нас пока `{}`);
  - в `doctors` — только врачи с интервалами; в `intervals` — free и занятые слоты;
  - для выгрузки только цен допустимы пустые `doctors: {}` и `devices: {}`.
  Успех — HTTP 204.
- HTTP 204 ≠ сразу видно на ПроДокторов. В ЛК МедФлекса нужно:
  1. «Ваша МИС» → «Прайс-лист» — убедиться, что прайс подтянулся (при необходимости «Обновить»);
  2. «Магазин интеграций» → «Выгрузка прайс-листа» → «Прайс-лист на ПроДокторов» —
     для филиала/категорий выбрать «Цена» или «Цена, расписание»;
  3. при необходимости в ЛК ПроДокторов: «Услуги и диагностика» → «Привязать услугу».
  Появление на сайте — до суток.
